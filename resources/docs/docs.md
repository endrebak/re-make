...
The `home-routes` handler in the `re-make.routes.home` namespace
...
The `home-page` function will in turn call the `re-make.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/re_make/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `re-make.handler` namespace:
...
Request middleware functions are located under the `re-make.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `re-make.dev-middleware` namespace found in
